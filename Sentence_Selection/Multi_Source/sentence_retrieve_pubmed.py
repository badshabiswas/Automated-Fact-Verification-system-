...
# 1. Load all claims from the JSONL file
file_path = config.get_dataset_path('scifact', 'Dev', 'dev.csv')
df = pd.read_csv(file_path)

# Extract claims as a list
all_claims = df['claim'].tolist()
print(f"Loaded {len(all_claims)} claims.")

# 2. Load the data using ast.literal_eval for non-standard JSON (like single quotes)
pmid_file_path = config.get_dataset_path('scifact', 'Dev/Relevant Doc/Pubmed', 'bm25_Pubhealth_pmids_2024_Original.txt')
...
output_file_path = config.get_dataset_path('scifact', 'Dev/Evidence Sentences/Pubmed', 'SciFact_joint_lines_Original_pubmed.txt')
