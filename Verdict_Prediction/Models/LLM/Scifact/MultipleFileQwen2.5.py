...
# List of input files
input_files = [
    config.get_dataset_path('SciFact\'.lower(), \'Dev/Evidence Sentences/Pubmed/Final Result/SciFact_joint_lines_Original_pubmed.txt\'),
    config.get_dataset_path('SciFact\'.lower(), \'Dev/Evidence Sentences/Pubmed/Final Result/Original_Pubmed_search_results_merged.txt\')
   
]

# Corresponding output and error log files (one for each input file)
output_files = [
    config.get_dataset_path('SciFact\'.lower(), \'Dev/Result/Pubmed/Qwen2.5/SciFact_Pubmed_classified_claims_original.txt\'),
    config.get_dataset_path('SciFact\'.lower(), \'Dev/Result/Pubmed/Qwen2.5/SciFact_Pubmed_classified_claims__merged.txt\')
]

error_log_files = [
    config.get_dataset_path('SciFact\'.lower(), \'Dev/Result/Pubmed/Qwen2.5/SciFact_Pubmed_error_log_original.txt\'),
    config.get_dataset_path('SciFact\'.lower(), \'Dev/Result/Pubmed/Qwen2.5/SciFact_Pubmed_error_log__merged.txt\')
]
...
def extract_classification_with_regex(response_content):
    """Extract classification character (A, B, C) from the model's response."""
    match = re.search(r"(?:Answer:\s*)?([ABC])", response_content)
    if match:
