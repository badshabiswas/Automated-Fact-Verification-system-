...
joint_lines_file = config.get_dataset_path('SciFact\'.lower(), \'Dev/Evidence Sentences/Pubmed/Final Result/Original_Pubmed_search_results_merged.txt\')
output_file = config.get_dataset_path('SciFact\'.lower(), \'Dev/Result/Pubmed/Llama-70B/SciFact_Pubmed_classified_claims_merged.txt\')
error_log_file = config.get_dataset_path('SciFact\'.lower(), \'Dev/Result/Pubmed/Llama-405B/SciFact_Pubmed_error_log_merged.txt\')
...
def classify_claim_evidence(claim, evidence):
    ...
    prompt = (
        ...
        "Please answer just A or B or C. Please refrain from generating any other words other than A or B or C: "
    )
    # Tokenize
