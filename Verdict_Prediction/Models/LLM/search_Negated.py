...
# Path to your text file containing claims
claims_file_path = config.get_dataset_path('scifact', 'data', 'negated_claims_dev.txt')

# Read claims from the text file
with open(claims_file_path, "r") as f:
    claims = [line.strip() for line in f if line.strip()]

# Search claims and save results
output_file = config.get_dataset_path('scifact', 'SciFact Relevant Doc/Dev/Wikipedia', 'search_results_Negated.json')
